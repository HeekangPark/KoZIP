...
class KoZIP:
    ...
    def maskedZIPtoAddr(self, masked_zipcode, masked_letter="*", depth=2, format="string"):
        assert type(masked_letter) is str and len(masked_letter) == 1, "Invalid param: masked_letter"
        assert type(masked_zipcode) is str and len(masked_zipcode) == 5, "Invalid param: masked_zipcode"
        assert len([x for x in masked_zipcode if not (x.isdigit() or x == masked_letter)]) == 0, "Invalid param: masked_zipcode"
        assert depth in [1, "1", "시도", 2, "2", "시군구", 3, "3", "도로명", 4, "4", "full"], "Invalid param: depth"
        assert format in ["list", "string"], "Invalid param: format"
        ...
        if masked_type not in self.new_zipcode_masked.keys():
            ...
            for key, value in data.items():
                masked_key = "".join([("0" if c.isdigit() else "*") for c in key])
        ...
        data = self.new_zipcode_masked[masked_type]
        return self._getAddr(data, masked_zipcode, depth, format)        

if __name__ == "__main__":
    kozip = KoZIP()
    print(kozip.maskedZIPtoAddr("08***", depth="시도"))